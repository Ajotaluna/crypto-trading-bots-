...
MIN_SCORE        = 130    # Score mínimo para incluir un par
KLINES_LIMIT     = 200    # Velas a descargar por par
KLINES_INTERVAL  = '15m'  # Intervalo de las velas
CONCURRENCY      = 3      # Descargas paralelas
INTER_BATCH_WAIT = 1.5    # Segundos de pausa entre batches
...
def _make_pick(symbol: str, result: dict, quote_vol_m: float) -> Dict[str, Any]:
    """Crea el dict de pick en formato estándar."""
    return {
        'symbol':        symbol,
        'score':         result['score'],
...
async def _get_universe(market) -> List[str]:
    ...
    # Fallback REST con reintentos y backoff exponencial
    ...
    logger.error("❌ No se pudo obtener el universo tras 3 intentos. Retornando lista vacía.")
    return []
...
async def _analyze_batch(
    ...
    """
    Descarga klines, contexto on-chain y calcula whale_score.
